...
///// Handles ,mutliple CSV File uploads in one argument ; i.e. CSVParse("1.CSV", "2.CSV", "3.CSV");
///// CSV Parser : Reads line by line, handles the line as data until the (,) then creates a new data field, reads ("") as string, moves to new row with \r\n 
///// Handles empty column exceptions, provides an <emptyrow> data input when column is empty
///// Handles empty/null line exceptions, ends read when line is null and skips line when line is empty
///// Read till the end of the CSV File and ends the process 
///// 

///// TODO:When it's reading from the CSV, we need to make sure that the data is being put in the right tables and columns. So before we export the Excel sheets to a CSV, we should rename the column names in a way to do this. My idea is to rename each column name like ... "<table-name> : <column-name>" ... and have the method sort the data based on that